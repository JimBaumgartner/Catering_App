...
event_controller = Blueprint('event_api', __name__)

@event_controller.route('/new', methods=['POST'])
@jwt_required
def create_event():
...
@event_controller.route('/all', methods=['GET'])
@jwt_required
def all_event():
    x = fetch_events()
...
@event_controller.route('/<int:post_id>', methods=['GET', 'PUT', 'DELETE'])
@jwt_required
